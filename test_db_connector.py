...
from db_connector import Enrolle
...
class TestDbConnector(TestCase):
    ...
    def test_insert_sql_script(self):
        cursor = db_connector.SqlExecutor('configs.txt')
        cursor.execute_sql_script('''
    ...
    def test_alter_sql_script(self):
        cursor = db_connector.SqlExecutor('configs.txt')
    ...
    def test_count_query(self):
        ...
        self.assertEqual(cursor.count_query(), 3)

    def test_min_max_query(self):
        cursor = SqlExecutor('configs.txt')
        self.assertEqual(cursor.min_max_query(), (382, 350))

    def test_sum_query(self):
        cursor = SqlExecutor('configs.txt')
        self.assertEqual(cursor.sum_query(), 350)

    def test_join_query(self):
        cursor = SqlExecutor('configs.txt')
        self.assertEqual(cursor.join_query(),
                         [Enrolle(1, 'Belevich', 'Mikhail', 'Andreevich', None, None, None, None, None, None, None),
                          Enrolle(2, 'Ishchenko', 'Ivan', 'Sergeevich', None, None, None, None, None, None, None),
                          Enrolle(3, 'Korolko', 'Olga', 'Yurievna', None, None, None, None, None, None, None)])
